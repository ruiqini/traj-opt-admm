...
namespace aabb
{
    ...
    std::vector<std::pair<unsigned int,unsigned int>> Tree::query( double margin)
    {
        std::vector<std::pair<unsigned int,unsigned int>> stack;
        stack.reserve(256);
        stack.push_back(std::make_pair(root, root));
        ...
        while (stack.size() > 0)
        {
            ...
            // Copy the AABB.
            AABB nodeAABB = nodes[node].aabb;
            AABB _nodeAABB = nodes[_node].aabb;
            ...
            if (_nodeAABB.overlaps(nodeAABB, touchIsOverlap, margin))
            {
                ...
                else
                {
                    if (nodes[node].isLeaf())
                    {
                       stack.push_back(std::make_pair(node,nodes[_node].left));
                       stack.push_back(std::make_pair(node,nodes[_node].right));
                    }
                    else if(nodes[_node].isLeaf())
                    {
                    ...
                    {
                       stack.push_back(std::make_pair(nodes[node].left,nodes[_node].left));
                       stack.push_back(std::make_pair(nodes[node].right,nodes[_node].right));
                       
                       stack.push_back(std::make_pair(nodes[node].left,nodes[_node].right));
                       stack.push_back(std::make_pair(nodes[node].right,nodes[_node].left));
                    }
