...
void log_data(std::string meshfile, Eigen::MatrixXd spline, double piece_time)
{
             ...
             for (double t = 0.0; t < piece_num; t += 0.05 / piece_time){
              ...
              result_file<<my_time.sum()<<" "<<len_ccd<<"\n";
...
void way_point_init(const std::string& mesh_file, int num_opt, std::vector<Eigen::Vector3d>& way_points)
{
    std::string line;
    std::ifstream myfile ("ompl_init/13/" +mesh_file + "_init_file"+std::to_string(num_opt)+".txt");
...
int main(int argc, char *argv[])
{
  ...
  result_file.open ("ompl_result/" +mesh_file + "_result_file_ompl.txt");
  ...
  */
  ...
  {
    way_point_init( mesh_file, num_opt, way_points);
