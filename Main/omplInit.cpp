...
int main(int argc, char *argv[])
{
  ...
  */
  std::vector<int> iter_size;
  iter_size.resize(20,0);
  iter_size[13]=400;
  for(int i=0;i<1000;i++)
  {
    ...
    if(size==13)
    {
      std::ofstream myfile("ompl_init/"+std::to_string(size)+"/" + mesh_file + "_init_file"+std::to_string(iter_size[size])+".txt");
      for(int i=0;i<way_points.size();i++)
      {
        myfile<<way_points[i].transpose()<<"\n";
      }
      iter_size[size]+=1;
