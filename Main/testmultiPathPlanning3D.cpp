...
void way_point_init(int i, std::vector<std::vector<Eigen::Vector3d>>& way_points_list)
{
    std::string line;
    std::ifstream myfile ("multi_init/init_file"+std::to_string(i)+".txt");
    //V*=0.5;
    Eigen::Vector3d p0;
    ...
    {
      int i=0;
      std::ifstream tmp_file("multi_init/init_file"+std::to_string(i)+".txt");
...
int main(int argc, char *argv[])
{
  ...
  result_file.open ("multi_result/result_file_multi.txt");
  ...
  //int k=atoi(argv[2]); 
  for(int num_opt=0;num_opt<600;num_opt++)
  {
    //int num_opt=
    std::cout<<"num_opt:"<<num_opt<<"\n";
    way_points_list.clear();
    way_point_init( num_opt, way_points_list);
    ...
    while(iter<num) 
    {
      ...
      if(iter>1 && gnorm<stop)
      {
            ...
            result_file<<whole_time<<" ";
            result_file<<piece_time*whole_weight<<" ";
            double whole_len=0;
  ...
  /*
  int k=atoi(argv[2]); 
  way_point_init( k, way_points_list);
      ...
      for(unsigned int k=0;k<subdivide_tree.size();k++)
      {
        ...
        for(int i=0;i<uav_num;i++)
        {
          ...
          else
          {
            for(int j=0;j<=order_num;j++)
            {
              viewer.data().add_edges(P[j], P[(j+1)%(order_num+1)], cc_list[cc_list.size()-1-k]);//Eigen::RowVector3d(0.8,0.2,0.8));
            }
          }
